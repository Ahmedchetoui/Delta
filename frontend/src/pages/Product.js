...
const Product = () => {
  ...
  const isPackMode = useMemo(
    () => currentProduct?.pricingMethod === 'pack' && Array.isArray(currentProduct?.packs) && currentProduct.packs.length > 0,
    [currentProduct]
  );

  useEffect(() => {
    if (!isPackMode) return;
    const defaultPack = currentProduct.packs.find((pack) => pack.isPopular) || currentProduct.packs[0];
    setSelectedPackId(defaultPack?._id || defaultPack?.title || null);
    setQuantity(defaultPack?.quantity || 1);
  }, [isPackMode, currentProduct]);

  const selectedPack = useMemo(() => {
    if (!isPackMode) return null;
    return currentProduct.packs.find(
      (pack) => String(pack._id || pack.title) === String(selectedPackId)
    ) || currentProduct.packs[0];
  }, [currentProduct, isPackMode, selectedPackId]);

  const handleSelectPack = (pack) => {
    setSelectedPackId(pack._id || pack.title);
    setQuantity(Number(pack.quantity) || 1);
  };
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        packId: isPackMode && selectedPack?._id ? selectedPack._id : undefined,
        shippingAddress: {
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center gap-3 mb-6 flex-wrap">
                <div className="text-3xl font-bold" style={{ color: '#B8860B' }}>
                  {isPackMode && selectedPack
                    ? (Number(selectedPack.price) % 1 === 0 ? selectedPack.price : Number(selectedPack.price).toFixed(2))
                ...
                {(isPackMode && selectedPack ? selectedPack.originalPrice : displayOriginalPrice) && (
                  <div className="text-xl text-gray-400 line-through">
                    {Number(isPackMode && selectedPack ? selectedPack.originalPrice : displayOriginalPrice) % 1 === 0
                      ? (isPackMode && selectedPack ? selectedPack.originalPrice : displayOriginalPrice)
                      : Number(isPackMode && selectedPack ? selectedPack.originalPrice : displayOriginalPrice).toFixed(2)} {t('currency')}
                  </div>
                )}
                {(isPackMode && selectedPack ? (selectedPack.badge || selectedPack.discount) : discountPercent) && (
                  <span className="px-2.5 py-1 bg-red-600 text-white text-xs font-bold rounded-full uppercase">
                    {isPackMode && selectedPack
                      ? (selectedPack.badge || `-${selectedPack.discount}%`)
                      : `-${discountPercent}%`}
                  </span>
                )}
              </div>
            ...
            <div className="space-y-6">
              {isPackMode ? (
                ...
                /* Quantité du mode standard existant */
              ...
              <div className="space-y-3 mb-6 text-sm">
                {isPackMode && selectedPack && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 space-y-1">
                    ...
                    {selectedPack.badge && (
                      <span className="inline-block rounded bg-emerald-500 px-2 py-0.5 text-[10px] font-extrabold text-white">{selectedPack.badge}</span>
                    )}
                  </div>
                )}
